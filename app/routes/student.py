...
@student_bp.route('/my/complaints/add')
@login_required
def add_complaint_page():
    """Student raise complaint page."""
    student = Student.query.filter_by(user_id=current_user.id).first()
    if not student:
        return render_template('students/no_profile.html')
    return render_template('complaints/student_add.html')


@student_bp.route('/my/complaints/add', methods=['POST'])
@login_required
def add_complaint():
    ...
    return redirect(url_for('student.complaints_page'))
